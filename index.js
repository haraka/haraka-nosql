...
function NoSQL(collection, options, done) {
  ...
  this.default_cb = options.done || function () {}
  ...
  switch (this.store) {
    ...
    case 'ssc': // Strong-Store-Cluster
      try {
        this.ssc = require('strong-store-cluster').collection(this.collection)
        if (this.expire) {
        ...
        console.error(e)
        console.log(
          'cannot load strong-store-cluster' +
...
NoSQL.prototype.del = function (key, done) {
  ...
  switch (this.store) {
    ...
    case 'ssc':
      this.ssc.acquire(key, function (err, keylock) {
        if (err) {
          console.error(err)
        }
        keylock.del()
...
NoSQL.prototype.redis_connect = function (done) {
  ...
  this.redis = redis.createClient({
    ...
    host: this.cfg.redis.host || 'localhost',
  })
